("Error himport java.io.*;
import java.net.*;
import java.util.*;
import java.util.concurrent.*;

public class Server2 {
    private static final int PORT = 5002; // Server2 port
    ...
    private static class ClientHandler implements Runnable {
        ...
        public void run() {
            try (
                ...
                System.err.printlnandling client in Server2: " + e.getMessage());
            } finally {
                try {
                    clientSocket.close();
                } catch (IOException e) {
                    System.err.println("Error closing client socket in Server2: " + e.getMessage());
                }
            }
        }
            ...
            synchronized (subscribers) {
                switch (subscriber.getStatus()) {
                    case "SUBS":
                        ...
                        output.writeObject("Subscriber added in Server2: " + subscriber);
                        break;
                    case "DEL":
                        subscribers.removeIf(s -> s.getId() == subscriber.getId());
                        output.writeObject("Subscriber removed in Server2: ID " + subscriber.getId());
                        break;
                    default:
                        output.writeObject("Unknown status in Server2: " + subscriber.getStatus());
                }
            }
        }
        ...
        private void handleAdminCommand(AdminCommand command, ObjectOutputStream output) throws IOException {
            if ("CAPACITY".equals(command.getCommand())) {
                ...
                output.writeObject("Unknown admin command in Server2: " + command.getCommand());
            }
        }
    }
}

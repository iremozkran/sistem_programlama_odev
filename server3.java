    ...
    private static class ClientHandler implements Runnable {
        ...
        public void run() {
            try (
                ...
                System.err.println("Error handling client in Server3: " + e.getMessage());
            } finally {
                try {
                    clientSocket.close();
                } catch (IOException e) {
                    System.err.println("Error closing client socket in Server3: " + e.getMessage());
                }
            }
        }
            ...
            synchronized (subscribers) {
                switch (subscriber.getStatus()) {
                    case "SUBS":
                        ...
                        output.writeObject("Subscriber added in Server3: " + subscriber);
                        break;
                    case "DEL":
                        subscribers.removeIf(s -> s.getId() == subscriber.getId());
                        output.writeObject("Subscriber removed in Server3: ID " + subscriber.getId());
                        break;
                    default:
                        output.writeObject("Unknown status in Server3: " + subscriber.getStatus());
                }
            }
        }
        ...
        private void handleAdminCommand(AdminCommand command, ObjectOutputStream output) throws IOException {
            if ("CAPACITY".equals(command.getCommand())) {
                ...
                output.writeObject("Unknown admin command in Server3: " + command.getCommand());
            }
        }
    }
}
